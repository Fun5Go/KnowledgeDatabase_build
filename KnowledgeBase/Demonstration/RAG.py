from KnowledgeBase.Generation.LLM_function import  failure_inference_generation_RAG, failure_inference_generation_PURE, failure_inference_generation_RAG_FILL
...
def RAG_pipeline(structure_input: Dict, KB_PATH: str, top_n: int = 25,top_k_per_field: int = 10,  
    ...
    if RAG:
        if not FILL:
            ...
            OUTPUT_PATH = Path(__file__).resolve().parent / "failure_candidates_RAG.json"
            ...
            OUTPUT_PATH = Path(__file__).resolve().parent / "failure_candidates_RAG_FILL.json"
    else:
       ...
       OUTPUT_PATH = Path(__file__).resolve().parent / "failure_candidates_pure.json"
