"""Evaluate extraction selection against selected_correct review labels.

The script reads the integrated human-review Top-60 JSON files and compares
``chunk_aggregate.selected`` with ``selected_correct``.

Confusion-count rules:
- TP: selected true and selected_correct true.
- TN: selected false and selected_correct null.
- FP: selected true and selected_correct null.
- FN: selected false and selected_correct true.

Chunks without a boolean ``chunk_aggregate.selected`` are skipped.
"""
...
DEFAULT_OUTPUT_JSON = SCRIPT_DIR / "extract_evaluation_top60.json"
DEFAULT_OUTPUT_CSV = SCRIPT_DIR / "extract_evaluation_top60.csv"
...
class QueryMetrics:
    ...
    f1: float = 0.0
    accuracy: float = 0.0
    true_positive_names: list[str] = field(default_factory=list)
...
def evaluate_file(path: Path) -> QueryMetrics | None:
    ...
    for index, chunk in enumerate(chunks, start=1):
        selected = chunk_selected(chunk)
        selected_correct = normalize_bool(chunk.get("selected_correct"))
        name = chunk_name(chunk, index)

        if selected is True and selected_correct is True:
            metrics.tp += 1
            metrics.true_positive_names.append(name)
        elif selected is False and selected_correct is None:
            metrics.tn += 1
            metrics.true_negative_names.append(name)
        elif selected is True and selected_correct is None:
            metrics.fp += 1
            metrics.false_positive_names.append(name)
        elif selected is False and selected_correct is True:
            metrics.fn += 1
...
def average_query_scores(results: list[QueryMetrics]) -> dict[str, Any]:
    ...
    if not evaluated:
        return {"precision": 0.0, "recall": 0.0, "f1": 0.0, "accuracy": 0.0, "num_queries": 0}
    return {
...
def evaluate_review_dir(review_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "confusion_rules": {
            "tp": "selected == true and selected_correct == true",
            "tn": "selected == false and selected_correct is null",
            "fp": "selected == true and selected_correct is null",
            "fn": "selected == false and selected_correct == true",
            "skipped": "chunks without a boolean selected value, or other selected_correct values",
        },
        "note": "FN uses selected_correct == true because TN already uses selected_correct null for selected false.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate extraction selected labels against selected_correct review labels."
    )
